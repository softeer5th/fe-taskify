...
function render(appNode) {
    ...
    // useWatch 실행
    useWatchStack.forEach((callbackF) => {
        callbackF()
    })
    useWatchStack.length = 0
}
...
function useWatch(callbackF, dependencies = []) {
    const node = currentNode
    ...
    if (node.useHook.current === null) {
        ...
        if (state.dependencies.length !==0 &&
            ...
            !state.dependencies.every((value, index) => value === dependencies[index])) {
            useWatchStack.push(callbackF)
        }
...
export {useState, useRef, useWatch}
