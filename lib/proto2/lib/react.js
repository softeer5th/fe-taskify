...
async function setState(newValue) {
    ...
    //effectObj.replace.add(this.node.parent)
...
// 유효성 검사 함수
function validateObjectLinks(obj) {
    // Helper 함수: 현재 객체의 children과 각 child's parent를 비교
    function isValidRelationship(parent) {
        if (!Array.isArray(parent.children)) {
            console.error(`Invalid children: ${parent.children}`);
            return false;
        }

        for (const child of parent.children) {
            // 자식의 parent가 현재 parent와 일치하지 않으면 오류
            if (child.parent !== parent) {
                console.error(`Mismatch found! Parent: ${parent}, Child: ${child}`);
                return false;
            }

            // 재귀적으로 자식의 관계도 검사
            if (!isValidRelationship(child)) {
                return false;
            }
        }
    ...
    // 시작 객체의 유효성 검사
    return isValidRelationship(obj);
}